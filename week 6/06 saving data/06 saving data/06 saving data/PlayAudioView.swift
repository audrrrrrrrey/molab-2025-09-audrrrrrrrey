...
//new code combines audio players for audio recorder and audio player demo
import SwiftUI
import AVFoundation
...
struct PlayAudioView: View {
    ...
    var body: some View {
        List {
            Section("preset sounds") {
                ForEach(bundleAudio, id: \.self) { file in
                    if let url = bundleURL(for: file) {
                        audioRow(for: url, label: file)
                    }
                }
            }
            
            Section("recorded sounds") {
                ForEach(audioRecorder.recordings, id: \.createdAt) { recording in
                    audioRow(for: recording.fileURL, label: recording.fileURL.lastPathComponent)
                }
            }
        }
        .listStyle(.insetGrouped)
        .navigationTitle("play audio")
        .onDisappear {
    ...
    //builds rows
    @ViewBuilder
    private func audioRow(for url: URL, label: String) -> some View {
        HStack {
            Text(label)
                .lineLimit(1)
            Spacer()
            Button {
                ...
                Image(systemName: currentFile == url && audioPlayer.isPlaying ? "stop.fill" : "play.fill")
                    .imageScale(.large)
            }
        }
    }
    ...
    private func togglePlay(for url: URL) {
        if currentFile == url && audioPlayer.isPlaying {
            audioPlayer.stopPlayback()
        } else {
